...
class AccountCreationScheduler:
    ...
    def run_account_creation(self):
        """执行账号创建任务"""
        try:
            logger.info("开始定时处理账号创建任务")
            call_command('process_account_creation_tasks')
            self.last_run = timezone.now()
            logger.info("定时处理账号创建任务完成")
        except Exception as e:
            logger.error(f"定时处理账号创建任务失败: {e}")
...
def run_account_creation_if_needed():
    ...
    if account_creation_scheduler.should_run():
        account_creation_scheduler.run_account_creation()